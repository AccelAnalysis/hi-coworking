 ...
 */
exports.team_create = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    const rfx = rfxSnap.data(); // RfxDoc
    // Verify territory status of the RFx
    let terrStatus = "released"; // Default if no fips
    ...
    if (!transactCheck.allowed) {
        throw new functions.https.HttpsError("permission-denied", `Cannot create team: ${transactCheck.reasons.join(", ")}`);
    }
    // 2. Create Team
 ...
 */
exports.team_invite = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in.");
    }
    const uid = context.auth.uid;
 ...
 */
exports.team_respond_invite = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in.");
    }
    const uid = context.auth.uid;
 ...
 * Prime only.
 */
exports.team_manage_member = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in.");
    }
    const uid = context.auth.uid;
    ...
    if (action === "remove") {
        ...
        if (scopeDescription !== undefined)
            members[memberIndex].scopeDescription = scopeDescription;
    }
    else {
